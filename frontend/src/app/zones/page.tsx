                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-white/[0.01] text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em]">
                                <th className="px-8 py-5">Название улицы / Сектора</th>
                                <th className="px-8 py-5">Цвет метки</th>
                                <th className="px-8 py-5">Тип геометрии</th>
                                <th className="px-8 py-5 text-right">Управление</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/[0.02]">
                            {zones.map((zone) => (
                                <tr key={zone.id} className="group hover:bg-white/[0.02] transition-colors">
                            ...
                            {zones.length === 0 && !loading && (
                                <tr>
                                    <td colSpan={4} className="px-8 py-20 text-center">
                                        <p className="text-[11px] font-black text-zinc-600 uppercase tracking-[0.3em]">Геозоны еще не созданы</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                {isEditModalOpen && editingZone && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
                        ...
                        <motion.div
                            ...
                            className="relative w-full max-w-md bg-zinc-900 border border-white/5 rounded-3xl p-8 shadow-2xl"
                        >
                            <h3 className="text-xl font-black text-white uppercase tracking-tight mb-6">Редактировать Зону</h3>
                            <form onSubmit={handleUpdateZone} className="space-y-6">
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-[#050505] border border-white/5 py-4 px-5 rounded-2xl text-white font-bold tracking-widest text-[11px] outline-none focus:border-emerald-500/50 transition-colors uppercase"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                ...
                                <div className="flex gap-4 pt-4">
                                    <button type="button" onClick={() => setIsEditModalOpen(false)} className="flex-1 py-4 font-black text-[10px] uppercase tracking-widest text-zinc-500 hover:text-white transition-colors">Отмена</button>
                                    <button type="submit" className="flex-1 bg-emerald-500 hover:bg-emerald-400 text-emerald-950 py-4 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-500/20 transition-all active:scale-95">Сохранить изменения</button>
                                </div>
                            </form>
                        </motion.div>
                    </div>
