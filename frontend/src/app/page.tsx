...
export default function Dashboard() {
  ...
  const handleDrawComplete = (polygon: any) => {
        ...
        <div className="flex-1 min-h-0 relative rounded-2xl overflow-hidden border border-white/5 shadow-2xl glass no-scrollbar">
          ...
          <div className="absolute top-4 right-4 flex flex-col gap-2 z-10">
            {!isDrawing && !isSavingZone ? (
              ...
              <button onClick={() => setIsDrawing(false)} className="bg-rose-500 hover:bg-rose-400 text-white px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 transition-all shadow-lg">
                <LogOut className="w-4 h-4 rotate-90" /> Отмена
              </button>
            ) : null}
          </div>
